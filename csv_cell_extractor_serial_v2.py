# vanilla python .csv tool: cell extraction
# row by index, to row as list of strings, to cell in row
...
def ensure_x_directory(directory_name_input=None):
    """
    Creates a x directory if it doesn't exist.
    ...
    returns:
        ...
        false if fail
    """
    if not directory_name_input:
        directory_name_input = "tmp"
        print("warning, no directory name input, defaulting to 'tmp/' ")
    
    try:
        Path(directory_name_input).mkdir(exist_ok=True)
        return True
